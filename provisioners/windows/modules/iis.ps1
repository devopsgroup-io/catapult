...
if (-not($configuration.websites.iis)) {
    ...
    for ($i=0; $i -le 10; $i++) {
        start-process -filepath "c:\Program Files\Git\bin\ssh-keyscan.exe" -argumentlist ("-4 -T 10 bitbucket.org") -Wait -RedirectStandardOutput $provision -RedirectStandardError $provisionError
    ...
    for ($i=0; $i -le 10; $i++) {
        start-process -filepath "c:\Program Files\Git\bin\ssh-keyscan.exe" -argumentlist ("-4 -T 10 github.com") -Wait -RedirectStandardOutput $provision -RedirectStandardError $provisionError
