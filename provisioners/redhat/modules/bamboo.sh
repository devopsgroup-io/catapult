...
if [ ! -d /usr/local/src/bamboo/atlassian-bamboo-5.15.3/atlassian-bamboo ]; then

    mkdir --parents /usr/local/src/bamboo
    cd /usr/local/src/bamboo
    
    curl --silent --show-error --connect-timeout 5 --output bamboo.tar.gz --retry 5 --location --url https://www.atlassian.com/software/bamboo/downloads/binary/atlassian-bamboo-5.15.3.tar.gz
...
sudo cat > "/usr/local/src/bamboo/atlassian-bamboo-5.15.3/atlassian-bamboo/WEB-INF/classes/bamboo-init.properties" << EOF
...
sed --in-place 's/port="8085"/port="80"/g' /usr/local/src/bamboo/atlassian-bamboo-5.15.3/conf/server.xml
...
# start bamboo
#bash /usr/local/src/bamboo/atlassian-bamboo-5.15.3/bin/start-bamboo.sh
