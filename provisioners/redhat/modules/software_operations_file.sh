...
if hash composer 2>/dev/null && hash drush 2>/dev/null && hash wp-cli 2>/dev/null; then
    ...
    if [ "${software}" = "codeigniter2" ]; then
        ...
        if [[ "${softwareversion_array[@]}" =~ "$(grep --only-matching --regexp="^[0-9]" <<< "${version}")" ]]; then
            ...
            if [ "${software_auto_update}" = "true" ]; then
                cd "/var/www/repositories/apache/${domain}/${webroot}${softwareroot}" && /opt/rh/rh-php56/root/usr/bin/php admin/cli/maintenance.php --enable
                ...
                cd "/var/www/repositories/apache/${domain}/${webroot}${softwareroot}" && /opt/rh/rh-php56/root/usr/bin/php admin/cli/upgrade.php --non-interactive
                cd "/var/www/repositories/apache/${domain}/${webroot}${softwareroot}" && /opt/rh/rh-php56/root/usr/bin/php admin/cli/maintenance.php --disable
            fi
