    ...
    configuration["websites"]["#{service}"].each do |instance|
      ...
      unless "#{instance["software"]}" == ""
        ...
        if "#{instance["software_dbprefix"]}" == ""
          catapult_exception("There is an error in your configuration.yml file.\nThe software for websites => #{service} => domain => #{instance["domain"]} requires the software_dbprefix option to not be \"\". Please remove the option if you do not need a software_dbprefix.")
        end
